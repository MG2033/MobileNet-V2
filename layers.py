...
class InvertedResidual(nn.Module):
    def __init__(self, in_channels, out_channels, expansion_factor=6, kernel_size=3, stride=2, bias=False):
...
def bottleneck_sequence(in_channels, out_channels, num_units, expansion_factor=6, kernel_size=3, initial_stride=2,
                        bias=False):
    bottleneck_arr = [InvertedResidual(in_channels, out_channels, expansion_factor, kernel_size, initial_stride, bias)]

    for i in range(num_units - 1):
        bottleneck_arr.append(InvertedResidual(out_channels, out_channels, expansion_factor, kernel_size, 1, bias))

    return nn.Sequential(*bottleneck_arr)


def conv2d_bn_relu(in_channels, out_channels, kernel_size=3, stride=2, dropout_prob=0.0, bias=False):
